...
SimilarTracks::SimilarTracks(QObject *parent, int limit) : QObject(parent) {
  numberOfSimilarTracksToLoad =
      limit; // this is also hooked to setting's number spinner in MainWindow;
  ...
  if (!file.open(QIODevice::ReadOnly)) {
    ...
    remixData = QString(file.readAll())
                    .trimmed()
                    .split(":DATA:")
                    .last()
                    .split("\"<<\"");
...
void SimilarTracks::addRemixes(QString songId) {
  ...
  // replace content written inside brackets of title if its not (feat. ... or (
  // feat..... this is to remove irrelivent content to make title clear. do not
  // remove if track is soundcloud cause there is artist infos there
  if (!isSoundcloudTrack && title.contains("(", Qt::CaseInsensitive) &&
      (!title.contains("(feat", Qt::CaseInsensitive) ||
       !title.contains("( feat", Qt::CaseInsensitive))) {
      ...
      m_netwManager, &QNetworkAccessManager::finished, [=](QNetworkReply *rep) {
        if (rep->error() == QNetworkReply::NoError) {
          ...
          foreach (QString str, list) {
            ...
            QString trackId =
                str.split("!=-=!")[4]; // track id of track being added to list
            // qDebug()<<title<<trackId;
            // prevent adding already played tracks or add tracks only which are
            // not played in this session
            if (!playedTracksIds.contains(trackId) && trackId != songId &&
                (remixIsValid(title))) {
              QString dataStr =
                  QString(str.split(");\">").first()).remove("&quot;");
              if (!dataStr.isEmpty() && (QString(dataStr.at(0)) == "'" ||
                                         QString(dataStr.at(0)) == "\"")) {
          ...
          if (finalList.count() > 0) {
            ...
            parentSongId =
                ""; // empty the parentsongid so user can reload request
            emit failedGetSimilarTracks();
          }
        } else {
          parentSongId = ""; // empty the parentsongid so user can reload
                             // request
          emit failedGetSimilarTracks();
        }
      });
  QUrl url =
      "http://ktechpit.com/USS/Olivia/manual_youtube_search.php?query=" + title;
...
void SimilarTracks::addSimilarTracks(QString video_id, QString songId) {
      ...
      m_netwManager, &QNetworkAccessManager::finished, [=](QNetworkReply *rep) {
        if (rep->error() == QNetworkReply::NoError) {
          ...
          if (finalList.count() > 0) {
            ...
            int tracksLeft;
            if (finalList.count() <
                numberOfSimilarTracksToLoad) { // set finalLeft tracks for the
                                               // for loop below
            ...
            for (int i = 0; i < tracksLeft; i++) {
              playedTracksIds.append(
                  QString(finalList.at(i)).split("!=-=!")[4]);
            }
          } else {
            parentSongId =
                ""; // empty the parentsongid so user can reload request
  ...
  if (albumId.contains("soundcloud-")) {
    url = "http://ktechpit.com/USS/Olivia/soundcloud/"
          "soundcloud_related_videos.php?query=" +
          store_manager->getTrack(songId).at(1);
  } else if ((albumId.contains("undefined-")) || isNumericStr(songId)) {
    url =
        "http://ktechpit.com/USS/Olivia/youtube_related_videos.php?video_id=" +
        video_id;
  } else {
    url = "http://ktechpit.com/USS/Olivia/spotify/"
          "spotify_related_seed_song.php?s_id=" +
          songId + "&url=null";
  }
  qDebug() << "URL USED:" << url;
  QNetworkRequest request(url);
...
void SimilarTracks::getNextTracksInPlaylist(
    QStringList trackListFromMainWindow) {
...
QList<QStringList> SimilarTracks::get_local_saved_tracks_data_only(int offset) {
  ...
  query.exec("SELECT trackId FROM tracks WHERE downloaded = 1 ORDER BY title "
             "ASC LIMIT " +
             QString::number(numberOfSimilarTracksToLoad) + " OFFSET " +
             QString::number(offset));
  while (query.next()) {
    trackList.append(
        store_manager->getTrack(query.value("trackId").toString()));
  }
  return trackList;
}
...
QString SimilarTracks::get_local_saved_tracks_PageNumber(int pageNumber) {
  ...
  if (totalPages > 0) {
    ...
    foreach (QStringList trackList, get_local_saved_tracks_data_only(offset)) {
      ...
      li += "gettrackinfo(&quot;" + title + divider + artist + divider + album +
            divider + "data:image/png;base64," + base64 + divider + songId +
            divider + albumId + divider + artistId + divider + "0" + divider +
            songId + ");\">";
    }
    html = li;
