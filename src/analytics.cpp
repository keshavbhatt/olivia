...
QString analytics::getData() {
  ...
  //    }
  return data;
  //"139.167.254.217<=>IN<=>x86_64<=>linux<=>4.4.0-157-generic<=>Ubuntu 16.04.6
  //LTS<=>1754332"
}

void analytics::headLessPush() {
  QUrl serviceUrl =
      QUrl("http://ktechpit.com/USS/Olivia/services/analytics.php");
  QByteArray postData;
  QUrlQuery query;
  ...
  args << "-q"
       << "--delete-after"
       << "--post-data" << QString(postData) << serviceUrl.toString();
