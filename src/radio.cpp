...
radio::radio(QObject *parent, int volumeValue, bool saveTracksAfterBufferMode)
    : QObject(parent) {
  ...
  QString fifoFileName =
      QString::number(QDateTime::currentMSecsSinceEpoch()) + ".fifo";

  // delete fifoFiles which are older then 10hr
  QStringList fifoFileInfoList =
      dir.entryList(QDir::System); // filter devices files
  qint64 currentTime = QDateTime::currentMSecsSinceEpoch();
  foreach (QString fifoFileInfo, fifoFileInfoList) {
    if (currentTime -
            fifoFileInfo.remove(".fifo").remove("_videoPlayer").toLong() >
        36000000) {
...
int radio::volume_() const { return volume; }
...
void radio::startRadioProcess(bool saveTracksAfterBufferMode, QString urlString,
  ...
  connect(radioProcess, &QProcess::stateChanged,
  ...
  if (settUtils->settingsObj.value("visualizer").toBool()) {
    vis_arg = " --autofit=1000  --no-input-default-bindings --no-osc "
              "--no-config -wid=" +
              QString::number(this->parent()
                                  ->findChild<nowPlaying *>("nowplaying_widget")
                                  ->findChild<QLabel *>("visualizer")
                                  ->winId()) +
              " --script=" + setting_path + "/vis.lua";
  } else {
    vis_arg = "";
  }

  QString status_message_arg =
      vis_arg + " --term-status-msg='[olivia:][${=time-pos}][${=duration}][${="
                "pause}][${=paused-for-cache}][${idle-active}][${cache-"
                "buffering-state}%][${=demuxer-cache-duration}][${=seekable}][$"
                "{=audio-bitrate}][${seeking}]'"; //[${=eof-reached}]
  ...
  if (urlString.isEmpty()) {
    radioProcess->start(
        "bash", QStringList()
                    << "-c"
                    << "mpv " + status_message_arg +
                           " --keep-open --keep-open-pause=no  "
                           "--demuxer-max-back-bytes=5000000 --no-ytdl "
                           "--gapless-audio=yes --audio-display=no --no-video "
                           "--input-ipc-server=" +
                           used_fifo_file_path +
                           " --volume=" + QString::number(volume) + " --idle");
  } else {
      ...
      radioProcess->start(
          "bash", QStringList()
                      ...
                      << "mpv " + status_message_arg +
                             ...
                             "--no-video --input-ipc-server=" +
                             used_fifo_file_path + " --volume=" +
                             QString::number(volume) + " --idle");
    else
      radioProcess->start(
          "bash", QStringList()
                      << "-c"
                      << "wget -O - '" + urlString + "' | tee " + setting_path +
                             "/downloadedTemp/current.temp" + " | mpv " +
                             status_message_arg +
                             " --keep-open --keep-open-pause=no --no-ytdl "
                             "--gapless-audio=yes --audio-display=no "
                             "--no-video --input-ipc-server=" +
                             used_fifo_file_path + " --volume=" +
                             QString::number(volume) + " --idle -");
  ...
  connect(radioPlaybackTimer, &QTimer::timeout, [=]() {
    ...
    fifoEOF->start("bash", QStringList()
                               << "-c"
                               << "echo '{\"command\":[\"get_property\" , "
                                  "\"eof-reached\"]}' | socat - " +
                                      used_fifo_file_path);
    ...
    if (radioProcess->state() == QProcess::Running) {
      ...
      QString position, duration, paused, paused_for_cache, idle_active,
          cache_buffering_state, demuxer_cache_duration, seekable,
          audio_bitrate, seeking;
      QStringList items;
      ...
      if (!demuxer_cache_duration.isEmpty()) {
        emit demuxer_cache_duration_changed(
            static_cast<double>(demuxer_cache_duration.toDouble()),
            static_cast<double>(position.toDouble()));
      }
...
void radio::radioReadyRead() {
  ...
  if (output.contains("[olivia:]")) {
    // mpv sometimes sends output without ] in line this will append ] and fix
    // it
    ...
    QTextBrowser *console =
        this->parent()->findChild<QTextBrowser *>("console");
    static_cast<QTextBrowser *>(console)->append(output);
    if ((output.contains("error reading", Qt::CaseInsensitive) ||
         output.contains("unable to resolve host address",
                         Qt::CaseInsensitive) ||
         output.contains("Failed to recognize file format",
                         Qt::CaseInsensitive)) &&
        !output.contains("Seek failed")) {
      radioState = "failed";
      emit radioStatus(radioState);
    }
...
void radio::radioSeek(int pos) {
  ...
  fifo->start("bash",
              QStringList()
                  << "-c"
                  << "echo '{\"command\": [\"set_property\" ,\"time-pos\"," +
                         QString::number(pos) + "]}' | socat - " +
                         used_fifo_file_path);
  fifo->waitForStarted();
}
...
void radio::pauseRadio() {
  ...
  fifo->start(
      ...
      QStringList()
          << "-c"
          << "echo '{\"command\": [\"cycle\" , \"pause\"]}' | socat - " +
                 used_fifo_file_path);
  fifo->waitForStarted();
...
void radio::changeVolume(int val) {
  ...
  fifo->start("bash",
              QStringList()
                  << "-c"
                  << "echo '{\"command\": [\"set_property\" ,\"volume\"," +
                         QString::number(volume) + "]}' | socat - " +
                         used_fifo_file_path);
}
...
void radio::quitRadio() {
  if (radioProcess != nullptr) {
    if (radioPlaybackTimer->isActive() ||
        radioProcess->state() == QProcess::Running) {
      radioState = "exit";
      emit radioStatus(radioState);
      radioPlaybackTimer->stop();
...
void radio::killRadioProcess() {
  if (radioProcess->state() == QProcess::Running) {
    QProcess::execute(
        "pkill", QStringList()
                     << "-P" << QString::number(radioProcess->processId()));
    if (radioProcess)
      delete radioProcess;
  }
}
...
void radio::stop() {
  QProcess *fifo = new QProcess(this);
  connect(fifo, SIGNAL(finished(int)), this, SLOT(deleteProcess(int)));
  fifo->start("bash", QStringList()
                          << "-c"
                          << "echo '{\"command\": [\"stop\"]}' | socat - " +
                                 used_fifo_file_path);
  fifo->waitForStarted();
  radioPlaybackTimer->stop();
}
