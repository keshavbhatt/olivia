...
Backup::Backup(QWidget *parent, QString settings_path, QSettings *settingsObj)
    : QWidget(parent), ui(new Ui::Backup) {
  ui->setupUi(this);
...
void Backup::fixTheme() {
  foreach (QLabel *label, this->findChildren<QLabel *>()) {
    if (label->objectName().contains("styled_", Qt::CaseInsensitive) ||
        label->objectName() == "last") {
...
void Backup::check_last_backup() {
  QString backupFilePath =
      ui->backup_path->text() + "Olivia_backup_" + lastBackTime + ".tar";
...
void Backup::on_backup_clicked() {
  if (backup_path_valid()) {
    ...
    args << "-cvf"
         << ui->backup_path->text().trimmed() + "Olivia_backup_" +
                lastBackTime + ".tar";
    if (!audio) {
...
void Backup::tarFinished(int exitCode) {
  ...
  if (exitCode == 0) {
    ...
    QMessageBox msgBox;
    msgBox.setText("An error occured while backing up data. Please report this "
                   "to developer.");
    msgBox.setIconPixmap(
        QPixmap(":/icons/sidebar/info.png")
            .scaled(42, 42, Qt::KeepAspectRatio, Qt::SmoothTransformation));
...
void Backup::on_change_path_clicked() {
  QString path = QFileDialog::getExistingDirectory(
      this, "Select path to store backup file", QDir::homePath(),
...
void Backup::on_restore_clicked() {
  ...
  msgBox.setText(
      "Warning: Pressing Ok button will delete current application data of "
      "Olivia and restore selected backup.\n\nOnly click Ok if you have "
...
void Backup::start_restore() {
  ...
  // get backup file
  QString backup_file_path = QFileDialog::getOpenFileName(
      this, tr("Select backup file to be restored."), back_dir_path,
      tr("Archive (*.tar)"), nullptr, QFileDialog::DontUseNativeDialog);
...
void Backup::untarFinished(int exitCode) {
  ...
  if (exitCode == 0) {
    ...
    QMessageBox msgBox;
    msgBox.setText("An error occured while restoring data. Please report this "
                   "to developer.");
    msgBox.setIconPixmap(
        QPixmap(":/icons/sidebar/info.png")
            .scaled(42, 42, Qt::KeepAspectRatio, Qt::SmoothTransformation));
