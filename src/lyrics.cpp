...
Lyrics::Lyrics(QWidget *parent) : QWidget(parent), ui(new Ui::Lyrics) {
  ui->setupUi(this);
  ...
  connect(_networkManager, SIGNAL(finished(QNetworkReply *)), this,
          SLOT(_networkManagerFinished(QNetworkReply *)));
...
void Lyrics::_networkManagerFinished(QNetworkReply *reply) {
  ...
  foreach (const QJsonValue &value, jsonArray) {
    if (this->isVisible()) {
      ...
      connect(lyricItem_Ui.showLyrics, SIGNAL(clicked(bool)), this,
              SLOT(showLyrics()));
...
void Lyrics::setLyricsTitle(QString title) {
  ui->lyricsWidget->findChild<ElidedLabel *>("lyricsTitleWidget")
      ->setText(title);
}
...
void Lyrics::showLyrics() {
  ...
  setLyricsTitle(
      senderButton->parent()->findChild<ElidedLabel *>("lyric_title")->text());
...
void Lyrics::showLyricsWidget(QString data) {
  ...
  QPropertyAnimation *animation =
      new QPropertyAnimation(ui->lyricsWidget, "maximumHeight");
...
void Lyrics::hideResult() {
  QPropertyAnimation *animation =
      new QPropertyAnimation(ui->resultWidget, "maximumHeight");
  animation->setDuration(500);
...
Lyrics::~Lyrics() { delete ui; }

void Lyrics::setCustomStyle(QString searchStyle, QString listStyle,
                            QString windowStyle) {
...
void Lyrics::on_query_returnPressed() {
  QNetworkRequest req;
  req.setUrl(QUrl("http://ktechpit.com/USS/Olivia/lyrics/search.php?query=" +
                  ui->query->text().trimmed()));
...
void Lyrics::on_close_clicked() { hideLyrics(); }
...
void Lyrics::on_lyrics_textChanged() {
  if (ui->lyrics->toPlainText().contains("Submit Now")) {
    ui->lyrics->setHtml(
        "<center><br><br><br>Lyrics for this song is not available! "
        "<br><br>"
...
void Lyrics::on_zoomIn_clicked() { ui->lyrics->zoomIn(1); }

void Lyrics::on_zoomOut_clicked() { ui->lyrics->zoomOut(1); }
