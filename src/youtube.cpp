...
Youtube::Youtube(QObject *parent, QWebView *webview,
                 paginator *pagination_manager)
    : QObject(parent) {
  setParent(parent);
  view = webview;
...
void Youtube::flat_playlist(QVariant playlist_id) {
  ...
  isOffline =
      pagination_manager->isOffline("youtube_playlist", "open_playlist", p_id);
  qDebug() << "playlist" << p_id << "is offline:" << isOffline;
  if (isOffline) {
    processPlaylistData(
        pagination_manager->load("youtube_playlist", "open_playlist", p_id));
  } else {
    ...
    flatter->start("python3", QStringList() << setting_path + "/core"
                                            << "--dump-single-json"
...
void Youtube::flatterFinished(int exitCode) {
  ...
  if (exitCode == 0) {
    ...
    if (flatter != nullptr) {
      ...
      uploader_url = "search.php?channelId=" + uploader_id +
      ...
      if (jsonArray.count() > 0) {
        foreach (const QJsonValue &value, jsonArray) {
          ...
          QString videoId, title, artist, album, thumb, songId, artistId,
              albumId, trackTimeMillis;
          ...
          if (title.contains("[Deleted video]", Qt::CaseInsensitive) ||
            ...
            list_items.append("<a data-channelhref='" + uploader_url +
                              "' onclick='gettrackinfo(\"" + title + "!=-=!" +
                              artist + "!=-=!" + album + "!=-=!" + thumb +
                              "!=-=!" + songId + "!=-=!" + albumId + "!=-=!" +
                              artistId + "!=-=!" + trackTimeMillis + "!=-=!" +
                              videoId + "\")'>");
            list_items.append("<img id='" + videoId +
                              "' style='max-width:250px  !important "
                              ";max-height:143px' src='" +
                              thumb + "'>");
            list_items.append(
                "<p style='margin-left:13.5em  !important;height:100px;'>");
            list_items.append("<b>" + title + "</b><br>");
            list_items.append(
                "<br><i class='ellipsis'>Added to: " + playlist_name +
                " <br> by: " + artist + "</i><br></p></a>");
            list_items.append("<a href='#' onclick='track_option(\"" + videoId +
                              "\")'>More Options</a>");
            list_items.append("<span>");
            list_items.append(
                "<a style='right: 3.8em;' onclick='watch_video(\"" + videoId +
                "\")' id='" + videoId +
                "_watchVideo' title='Watch Video' class='ui-li-count "
                "my-watch-video-button' href='#' data-role='button' "
                "data-iconpos='notext' data-icon='video' data-theme='b'>");
            list_items.append("Watch</a></span>");
            list_items.append("</li>");
          }
        }
        QString header =
            "<div id='HEADER_DIV' style='height:250px;background-image:url(" +
            QString("\"" + playlist_thumb + "\"") +
            "); background-size: cover;"
            "text-align:center; background-position: center;text-shadow: 1px "
            "3px 14px rgb(0, 0, 0);"
            "box-shadow:rgb(0, 0, 0) 0px 0px 30px;'>"
            "   <div id='HEADER_WRAPPER' "
            "style='height:100%;background-color:rgba(0, 0, 0, 0.27);' >"
            "      <h2 style='font-size: 2em;position: relative;top: "
            "35%;margin: 0px auto;"
            "         background-color: rgba(0, 0, 0, 0.49);padding: 20px "
            "0px;'>" +
            playlist_name +
            "</h2>"
            "   </div>"
            ...
            "<br>";
        QString filter =
            "<form class='ui-filterable'><input class='ui-icon-alt'  "
            "id='resultfilter-input' data-type='search' placeholder='Filter "
            "tracks in playlist..'></form>";
        html.append(header);
        html.append("<i style='display: block;text-align:center;' "
                    "class='ellipsis'>Video count: " +
                    QString::number(videoAdded) + " </i>");
        html.append(filter);
        html.append(
            "<ul data-filter='true' data-input='#resultfilter-input' "
            "class='list' id='manul_youtube_page_result'  data-role='listview' "
            "data-split-icon='bars' data-split-theme='b' data-inset='true'>");
        html.append(list_items);
        html.append("</ul>");

        // save in paginator
        pagination_manager->save("youtube_playlist", "open_playlist",
                                 playlist_id, html);
