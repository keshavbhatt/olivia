...
// used to save data returned from function dataType of page pageType for query
// query
void paginator::save(QString pageType, QString dataType, QString query,
                     QString data) {
  ...
  QString loadOnlineBtn =
      ...
      "' style='padding-top: 10px;margin: 10px 0px;"
      "background-color: rgba(36, 142, 179, 0.54);border: none;'"
      " class='ui-mini ui-shadow-icon ui-btn ui-shadow ui-corner-all "
      "ui-icon-clock ui-btn-icon-left' "
      " onclick='paginator.deleteCache(\"" +
      pageType + "\",\"" + dataType + "\",\"" + query + "\")'> Updated on " +
      QDateTime::currentDateTime().toLocalTime().toString() + " - Reload</a>";

  QString timeagoScript =
      "<script>var date = Date.parse('" +
      QDateTime::currentDateTime().toLocalTime().toString() +
      "');"
      "$.mobile.activePage.find('#timeElement" +
      QString::number(QDateTime::currentMSecsSinceEpoch()) +
      "').text('Updated '+TimeAgo.inWords(date)+' - Reload');</script>";
  out << loadOnlineBtn << data.trimmed() + timeagoScript;
}

// used to load data from offline page pageType for funcction dataType for query
// query
...
void paginator::save_json(QString pageType, QString dataType, QString query,
                          QString data) {
  QString path = createDir(pageType.trimmed() + "/" + dataType.trimmed());
  QFile file(path + query.trimmed());
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
...
QString paginator::load_json(QString pageType, QString dataType,
                             QString query) {
  QFile file(getPath(pageType, dataType, query));
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    qDebug() << "unable to open file";
...
QString paginator::getPath(QString pageType, QString dataType, QString query) {
  QDir file(paginator_path + "/" + pageType.trimmed() + "/" +
            dataType.trimmed() + "/" + query.trimmed());
  // qDebug()<<file.path();
  return file.path();
}
...
QString paginator::getList(QString page, QString dataType) {
  QStringList files;
  files << "fake-item"
        << QDir(getPath(page, dataType, "")).entryList(QDir::Files, QDir::Time);
  ...
  while (numberOfButtons <= files.count()) {
    for (int f2 = 0; f2 < 3; f2++) {
      ...
      if (numberOfButtons >= 10)
        break;
      html.append("<div class='ui-block-" + col + "'><a onclick='" + dataType +
                  "(\"" + files.at(numberOfButtons) +
                  "\");$(\"#manual_search\").val(\"" +
                  files.at(numberOfButtons) +
                  "\")' class='ui-shadow-icon ui-btn ui-shadow ui-corner-all "
                  "ui-icon-clock ui-btn-icon-left ui-mini'>" +
                  files.at(numberOfButtons) + "</a></div>");
    }
    row++;
  }
  return html;
...
void paginator::clearRecentSearches() {
  ...
  filesInfo << QDir(getPath("youtube", "manual_youtube_search", ""))
