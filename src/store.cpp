...
void store::initStore(QString dbName) { // takes dbname only dont include paths
  ...
  if (!db_path.isEmpty()) {
    if (db.isOpen()) {
      qWarning() << "Store open for queries"
                 << setting_path + "/storeDatabase/" + dbName + ".db";
    } else if (!db.isOpen()) {
...
// creates storeDatabase file with dbname given in defined pAths and opens it
// for transactions
...
void store::createTable(QString dbName) {
  ...
  if (track_created && queue_created && album_created && artist_created &&
      color_created && ytIds_created && art_created && streamUrl_created) {
...
void store::createTableVersion(QString dbName, int version) {
  Q_UNUSED(dbName);
  // QString path =
  // QStandardPaths::writableLocation(QStandardPaths::DataLocation)+"/storeDatabase/"+dbName+"/";
...
// function to set NULL in downloaded cloumn , used by setting dialog for
// delete_cached_tracks;
  ...
  if (query.record().count() > 1) {
    while (query.next()) {
      ...
      if (file.remove()) {
        file.deleteLater();
        QSqlQuery query2;
        query2.exec("UPDATE tracks SET 'downloaded' = NULL WHERE trackId = '" +
                    trackName + "';");
      }
    }
  } else {
...
// saves radio channel to favourite in db with all columns in radio_favourite
// table in DB
void store::setRadioChannelToFavourite(QStringList meta) {
  //("87600", "http://stream-uk1.radioparadise.com/aac-320", "Radio Paradise
  //(320k)", "United States of America", "English\")")
  QString title = QString(meta.at(2)).remove("\"").replace("'", "''").trimmed();
  QString country =
      QString(meta.at(3)).remove("\"").replace("'", "''").trimmed();
  QString lang = QString(meta.at(4)).remove("\"").replace("'", "''").trimmed();
  QSqlQuery query;
  query.exec(
      "INSERT INTO radio_favourite('channelId','url','title','lang','country') "
      "VALUES('" +
      meta.at(0) + "','" + meta.at(1) + "','" + title + "','" +
      lang.remove(")").remove("(") + "','" + country + "')");
}

// check if is favourite channel
bool store::is_favourite_station(QString channel_id) {
  QSqlQuery query;
  query.exec("SELECT channelId FROM radio_favourite WHERE channelId = '" +
             channel_id.trimmed() + "';");
  if (query.next()) {
    return true;
  } else {
...
void store::removeRadioChannelFromFavourite(QVariant channelId) {
  QSqlQuery query;
  query.exec("DELETE FROM radio_favourite WHERE channelId= '" +
             channelId.toString().trimmed() + "';");
}

// check if is favourite channel
bool store::is_favourite_playlist(QVariant playlistId) {
  QSqlQuery query;
  query.exec("SELECT id FROM liked_playlists WHERE id = '" +
             playlistId.toString().trimmed() + "';");
  if (query.next()) {
    return true;
  } else {
    return false;
...
void store::removePlaylistFromFavourite(QVariant playlistId) {
  QSqlQuery query;
  query.exec("DELETE FROM liked_playlists WHERE id= '" +
             playlistId.toString().trimmed() + "';");
...
void store::setPlaylistToFavourite(QVariant meta_var) {
  ...
  QString metadata =
      QString(meta.at(3)).remove("\"").replace("'", "''").trimmed();
  QString vid_count =
      QString(meta.at(4)).remove("\"").replace("'", "''").trimmed();
  QString base64 = meta.at(5);
  QSqlQuery query;
  query.exec(
      "INSERT INTO "
      "liked_playlists('id','title','vid_count','by','meta','timestamp') "
      "VALUES('" +
      playlistId + "','" + title + "','" + vid_count + "','" +
      by.remove(")").remove("(") + "','" + metadata.remove(")").remove("(") +
...
void store::update_track(QString entity, QString trackId, QString value) {
  QSqlQuery query;
  query.exec("UPDATE tracks SET " + entity.remove("\"") + " = '" + value +
...
void store::saveAlbum(QString id, QString title) {
  QSqlQuery query;
  query.exec("INSERT INTO album('albumId','albumName') VALUES('" +
             id.trimmed()
                 .remove("\"")
                 .replace("'", "''")
                 .remove(QChar('\\'))
...
void store::saveArtist(QString id, QString title) {
  QSqlQuery query;
  query.exec("INSERT INTO artist('artistId','artistName') VALUES('" +
             id.trimmed() + "','" +
             title.trimmed().remove("\"").replace("'", "''") + "')");
}

// saves dominant color of earch album from its art
void store::saveDominantColor(QString albumId, QString value) {
  QSqlQuery query;
  query.exec("INSERT INTO color('albumId','colorName') VALUES('" +
             albumId.trimmed()
                 .trimmed()
                 .remove("\"")
                 .replace("'", "''")
...
void store::saveytIds(QString trackId, QString ids) {
  QSqlQuery query;
  bool saved = query.exec("INSERT INTO ytIds('trackId','ids') VALUES('" +
                          trackId.trimmed() + "','" + ids.trimmed() + "')");
  if (!saved) {
    QSqlQuery update;
    update.prepare("UPDATE ytIds SET ids='" + ids.trimmed() +
                   "' WHERE trackId='" + trackId.trimmed() + "'");
    update.exec();
  }
}
...
void store::saveArts(QString albumId, QString artId) {
  QSqlQuery query;
  query.exec("INSERT INTO arts('artId','albumId') VALUES('" +
             albumId.trimmed() + "','" + artId.trimmed() + "')");
}

// FILESYSTEM
void store::saveAlbumArt(QString albumId,
                         QString base64) { // save album art if not exists
  // add entry to db
  QString artId =
      "art-" +
      albumId.remove("\"").replace("'", "''").remove(QChar('\\')).remove("/");
  saveArts(
      artId,
      albumId.remove("\"").replace("'", "''").remove(QChar('\\')).remove("/"));
  // save to disk
...
// saves the url of track with expiryTime which is check by mAInWindow while
// loading track
void store::saveStreamUrl(QString songId, QString url_str, QString expiryTime) {
  QSqlQuery query;
  bool saved = query.exec(
      "INSERT INTO stream_url('trackId','url','timeOfExpiry') VALUES('" +
      songId.trimmed() + "','" + url_str.trimmed() + "','" +
      expiryTime.trimmed() + "')");
  // update track info if it already exist or any kind of error while inserting
  // new entry
  if (!saved) {
    QSqlQuery update;
    update.prepare("UPDATE stream_url SET url='" + url_str.trimmed() +
                   "',timeOfExpiry='" + expiryTime.trimmed() +
                   "' WHERE trackId='" + songId.trimmed() + "'");
    update.exec();
  }
}
...
QStringList store::getAlbumDetails(QString albumId) {
  ...
  return QStringList() << albumId << getAlbum(albumId) << getThumbnail(albumId)
                       << getDominantColor(albumId) << artistName << artistId
                       << tracksCount;
}
...
QStringList store::getTrack(QString trackId) {
  ...
  return QStringList() << trackId << title << albumId << getAlbum(albumId)
                       << artistId << getArtist(artistId)
                       << getThumbnail(albumId) << getOfflineUrl(trackId)
                       << getYoutubeIds(trackId) << getDominantColor(albumId);
}
QStringList store::getRadioStation(QString trackId) {
  ...
  return QStringList() << trackId << url << title << lang << country
                       << "qrc:/web/radio/station.jpg";
}
...
bool store::getExpiry(QString trackId) {
  ...
  if (!trackId.trimmed().isEmpty()) {
    QSqlQuery query;
    query.exec("SELECT timeOfExpiry FROM stream_url WHERE trackId = '" +
               trackId + "'");
...
QString store::web_print_saved_albums() {
  ...
  foreach (QStringList albumList, getAllAlbums()) {
    QString albumId, albumName, artistName, base64, dominantColor, artistId,
        tracksCount;
  ...
  if (untitledAlbumsRecordObjectCount > 0) {
    ...
    recordObject.insert(
        ...
        "AP+"
        "gvaeTAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAB3RJTUUH4wMHDiINVe6WcgAAAB1pVFh0Q"
        "29tbWVudAAAAAAAQ3JlYXRlZCB3aXRoIEdJTVBkLmUHAAAJ6UlEQVR42u2dW2wdVxWGv3/"
        "P2MdJb6EtTRNiHyc+SaWAoiIQhSJSH6cQWqFKSEjwhIC+"
        "FVTgBSF4RhTEQ6GNhCIhhPqAkKrSi9RC09gnCTcJVKAPQBvb8XFC2rROE5o09eXMXjzMjC"
        "9piJp0tj0Os6Sx53Lmstc/"
        "67rX7K1WvfEdSb0YhnBgAjkDBziBAAcWGYqU7TeIMYuASFKM0YXoBtYAPUANiEnPB/"
        "BAB5gDZrNl2rBpGTOIadAsMGPQEcwZ5gVmCJkZkjfwYB4jkfAgD1i2CIjAYlAXWFe2z877"
        "nQcSsPx8j5k3smuaEsMSIEF4jHR9oQ3JBRaPSLDsuJQAiZl5sI6MxKQEs0TpsQ5YYpb+"
        "TiLx5t+IJX0RuGWebdnK/CYL+7VkC5DO23HpJPS2c3XeHRffa/6vLn7Vd/"
        "ZQi59fS1iwpLW6pAYt3ZTmn/"
        "dtPENLNp2iYZchXlEJSGbTLhPhispB5ioelAiNDJBKQsqisiRzmYdQUTnIV0a9TCrLUpU1"
        "W7GiLCoL7zDeqlhRJi9LnK34UBpKHHCq4kNZ3F7zFSClitRlzrBKZZUGEbwDnas4UaY4xJ"
        "ip+"
        "FCuwHCu4kOpALEKkDLFIZIqQMokIWZW5bJKFalX2d7SAeIrPpSGkkpCyiQeWCUhZUOkAqR"
        "ElHfhVn3qpZEQM5dV7lVUDhHxTlglIWVye62yIaWiGC27DXkFGANOAm9mToVjoSo2L4bOC"
        "5s9kGQF2Pl+W2T78nMdaXF3TFro3UNa+N2dHfekidS5zNW3RedEi87NC8avBtYBNy+"
        "nmxVnVe/LYa/+gvip0AvAKe/"
        "9m6QVLwbIOTAjV6DzoAjMzHzHMJl5ImckiTnn8N6nmtchhwTOOWWV+"
        "VIs0W1GhNLqeQNvkDjMG87SSn8ihMPSqn5EJNSVgmprQNcAA8DdwD2h3d5YywP9D4A9U29"
        "MHfvc66dXnc16dkPv/"
        "lqt9qgZt0v6EbBtNausPWb2wGB79DTAcN8WhibHVw0YI30DNCfHLFOxT43UG8ck7RV8uHg"
        "vK/"
        "0wJyT9A3h4sD16ulVvAKwqMACak2ML4NQbNNujfxU8DLwRwO8ldPX7Q3dMHP4XwGB7dNV7"
        "QM2sDd773wB/D3EPF1BhHfLe78vF/kqhg43tNCfHThj8keLrohVSQv7cnBwbO1/"
        "sVzsl02mRjnmezFz4Ir0s5wK5vDMYYwCtK0g6AJrHJjLbcvj3wPFC8ZC5UBJyBjEFMFigd"
        "DxX3zK/"
        "3uprrJzaGti+EFsVqLaEolA25IwZrxV90Tvb4+zr39zd6m3cOjg5ykrZp7fOnsxX/"
        "wRMFxmGOCmI0jphZkHcqpm5Doo00qpvfejnoObkGLlLvVy0+"
        "8SJfPWfFFvX1uUC5Xpfbk6OHg2UggFYJ/HVLf1b/9aqN24dbI9i936K4d7llRYz/"
        "28K7QK3NU4KYtVfAxjeFJRBAnZIOtiqN+5v7Rtfm6ydWxLEBQekk/"
        "yHQrPl6nEWRkQMYOjYsri710j6sXN6pmumuzlSb1yVB3H7N9aD3njoePtcoYCY1RanvYsE"
        "YyWK73YKDUt6oFVvvB9g1/H2chj9It/oOAQgHlbuu0XB1yQ9cqC/"
        "8fk8KD24+RZ+vW5dqFsWJiEmRSHikA6w0t+cfBC090D/"
        "1gdH+gY27TzyIp89fZpWvThpeXr9+oUguLiXyYWQkMSMN0sQVF8LfN059/"
        "iB+tb7n+vd2D3YTm1aEc7G3Quub5Hf1yiIypJK9RHQhxA/7IquOjDSNzCUOxvD/"
        "Vs5VIx9KRSQmOIDQzMLl0OO/GU9bg34qHPut61642fAdwcnDp8sKlYtsn1ueXrUCxS/"
        "6bl3A3Ys8XGw9z15000UCEhRL2CQ4ZmEwoHsNXu5jffArxJvQ4PtsRfuefVVhvs2F/"
        "FIReayloyJWCAi4cbhkr+st3EK2NPpdL6/"
        "69iRmYP92zgzc4ahySNlU1k+VvG5E2coDqazapfUZeCBfYbtHZwYfQygVW+wc+KlIp+"
        "osMF7DLPYzhuWsQhAhNVC4dEVxe8UkN8Bj3jvn2xOjr2Sg1Hmvn2ZfIyZCoYkAq0N9dCz0"
        "51kzcXhfh34npk9MdgeHQM42L+VmU4SCoxagRLiQ8QhMXANwDM33lh46+"
        "85NXWxPNljZrar0+n8JAdjf+"
        "8AOycO88ljwcqPri1MQiQfwqgLuB7grqmpUEyw8577deBb3ie/"
        "bE6OL0nb7DoaJuO8v2+AXWn39PqieGjgYwhSm3VdYHWbZJI4Y2aPe++/"
        "PXR0fAJgpHcLzaPhi/"
        "F2LdQKFNnWxAUqBbpupK9xU0B+"
        "nDazJ5LEPjLYHv3C0NHxiTzNvhxg5DTcu6UvezGKCheSOFCf+vWS+"
        "oBXg4iHJbuH2uPPAxyo38L03JkVqf1ykdtO+"
        "vlCMXrY5J2ZhQBkvWSN7C0q9MIj9QGG2uPPH9q8jac2buSO9ovsPn58WYF4btOW3E29tUh"
        "AJJJQNuQGYDvAUMEqpJml0D9x5CVWiu5c8NhuK9LtzWxIqLyTdrT6Bt7LFUrDm/"
        "quRjQK5l/iRLAK+Nvk3Afgyiq2zju3XFy7Ayj0hTMjcQEzszeb2c5nN/TFV1KxdV5JI/"
        "hMFoMUp1NkibOA/"
        "SGS7q3VatthZWtxC3MoMklv1Rs7gNsDcKzjIOhHO72YfanVO3BVXov7zIbe1QdEVhHZnBz"
        "jF1wtSV8GdoTw6EMDAuI+RfpYq7+xFuCul4+uOkCaWfplpHdzXK9v+"
        "Apwb6gMRBwcEFQDHhXsbfVvPQRMyews0ltmNosxa87mMGbmjE4XGEkiL5OyqhhDXrLO7Ox"
        "M59Mnjl+0DuohoAd0Fuybl/CUT6/fFHXHcRxFUTdSl/"
        "LubUcXsFawHrQb+"
        "AZZ8rRoMixRq79xn9Ce5XrbDE4qjeBPkQ4ccI60sO6MwYzSPJUDi0ARZjLJK+"
        "2ZmybtEJpjYRCBbGa2+QEAFtvEfJABz4UHHcjP7coCvB5gbbbkL2tPFldty/"
        "6HpAdjDLecZQ5KG3XD/zj29i1pdVVhvGu3l/"
        "+b9paepDQOqSYGKw8l+RgfFZWDOiFTJxVduhHx+"
        "Zy3FZUBj2UJDCu65Ei98rJKBUjlZZVOQqKKD6WJQ3zl9pYrUveVUS+"
        "XjFhl1MulsswhVRJSnjgEp2qS+zIZEbnAo8pVdGkUOaskpEQ2RM6pkpAyqazK7S2ZUZ+"
        "tIvVy0bkq/"
        "V4umnaqJKRMZn26siHlsiJzFSDlIlcBUi6V1VMBUiY4sPdUbm+"
        "5QvUNDqNWcaIsgbrFsWGtrMC5kwvOO4opL0Mgc8OVjq+"
        "Sz66mXG06IDLDKZ3YKwKLZHImosw9z2dVyyX7/P/5ui6wXKgBtmjxF/i/"
        "eEkutJ1OO2jZjHLymKXH0wLx/"
        "HfZ9Sy9tsmQGciySdkMrBv4w38BGMjfMAjiDEYAAAAASUVORK5CYII=");
    recordObject.insert("dominantColor", "75,245,85");
    recordObject.insert("artistName", "Various");
    recordObject.insert("artistId", "undefined");
    recordObject.insert("tracksCount",
                        QString::number(untitledAlbumsRecordObjectCount));
    recordsArray.push_back(recordObject);
  }
...
//===========================================================PAGINATION FOR
//LOCAL SONGS=========================================================== get the
// records count in a given row of table
...
// main result provider update query in getTrackCount also to get correct page
// numbers
QList<QStringList> store::get_local_saved_tracks(int offset) {
  QSqlQuery query;
  QList<QStringList> trackList;
  query.exec("SELECT trackId FROM tracks WHERE downloaded = 1 ORDER BY title "
             "ASC LIMIT " +
...
QString store::open_local_saved_tracks_PageNumber(int pageNumber) {
  ...
  head = "<p style='margin-top: -5px;'>showing page " +
         QString::number(currentPageNumber + 1) + " of " +
         QString::number(totalPages) + " pages.</p>";
  script = "<script>$(\".ui-page-active [data-role='header'] h1\").html(\"" +
           QString::number(totalTracks) + " downloaded songs\");</script>";

  Next = "<a class='ui-btn ui-mini ui-icon-arrow-r ui-btn-icon-right ui-shadow "
         "ui-corner-all'" +
         QString(" style='float:right;width:40%;background-color: rgba(36, "
                 "142, 179, 0.66);border: none;'") +
         " id='navBtn'" + " onclick='openPagenumber(\"" +
         QString::number(currentPageNumber + 1) + "\")'" + ">Next</a>";
  Previous = "<a class='ui-btn ui-mini ui-icon-arrow-l ui-btn-icon-left "
             "ui-shadow ui-corner-all'" +
             QString(" style='float:left;width:40%;background-color: rgba(36, "
                     "142, 179, 0.66);border: none;'") +
             " id='navBtn'" + " onclick='openPagenumber(\"" +
             QString::number(currentPageNumber - 1) + "\")'" + ">Previous</a>";
  ...
  if (!pagination.trimmed().isEmpty()) {
    footer =
        "<div style='background-color: rgba(29, 29, 29, 0.64);' "
  ...
  if (totalPages > 0) {
    ...
    foreach (QStringList trackList, get_local_saved_tracks(offset)) {
      ...
      if (albumId.contains("undefined-")) {
        ...
        imgHtml = "<img id='" + songId +
                  "' "
                  "style='max-width:178px;max-height:144px;width=178px;height="
                  "100px;' id='' src='data:image/png;base64," +
                  base64 + "' />";
      } else {
        para = "<p style='margin-left: 14.5em;' >";
        imgHtml = "<p style='background-color:rgb(" + dominantColor +
                  ");' class='li-img-wrapper'><img id='" + songId +
                  "' "
                  "style='width:100%;max-width:100px;max-height:144px;width="
                  "100px;height=100px;' id='' src='data:image/png;base64," +
                  base64 + "' /></p>";
      }
      li += "<li data-filtertext='" + title + " " + album + " " + artist +
            "' >" + "<a data-trackinfo='" + title + divider + artist + divider +
            album + divider + base64 + divider + songId + divider + albumId +
            divider + artistId + divider + "millis" +
            "' onclick='mainwindow.playLocalTrack(\"" + songId + "\")'>" +
            imgHtml + para + "" + title + "<br>" +
            "Album: " + QString(album == "undefined" ? "Youtube" : album) +
            "<br>" + "Artist: " + artist + "</p>" + " </a>" +
            "<a href='#' onclick=\"track_option('" + songId +
            "')\">More Options</a>" + "</li>";
    }
    html = head +
           "<ul style='margin-bottom: 60px;'  class='list' "
           "id='saved_tracks_result'   data-role='listview' "
           "data-split-icon='bars' data-split-theme='b' data-inset='true'>" +
           li + "</ul>" + script + footer;
    return html;
  } else {
...
//===========================END PAGINATION FOR LOCAL
//SONGS========================//

//===========================START SONG FILTER
//METHOD====================================// gives the count of tracks for a
// query
int store::getSearchResultTrackCount(QString queryStr) {
  QSqlQuery query;
  query.exec("SELECT trackId FROM tracks WHERE title LIKE '%" + queryStr +
             "%' AND downloaded = '1';");
  int count = 0;
  while (query.next()) {
    QString trackId = query.value("trackId").toString();
...
// main result provider update query in getSearchResultTrackCount also to get
// correct page numbers
QList<QStringList> store::get_search_local_saved_tracks(int offset,
                                                        QString queryStr) {
  QSqlQuery query; //,artistIdquery;
  QList<QStringList> trackList;
  // QStringList artistMatched;
  query.exec("SELECT trackId FROM tracks WHERE title LIKE '%" + queryStr +
             "%' AND downloaded = '1' LIMIT " + QString::number(limit) +
  ...
  //    artistIdquery.exec("SELECT artistId FROM artist WHERE artistName LIKE
  //    '%"+queryStr+"%';");
  ...
  //            if(trackIdquery.exec("SELECT trackId FROM tracks WHERE artistId
  //            = '"+artistId+"';")){
...
// open page number usually called from next prev buttons
QString store::open_search_local_saved_tracks(int pageNumber,
                                              QVariant queryStr) {
  currentPageNumber = pageNumber;
  QString Next, Previous, pagination, script, head, footer;
  head = "<p style='margin-top: -5px;text-align:centre;'>Showing results on "
         "page " +
         QString::number(currentPageNumber + 1) + " of " +
         QString::number(totalPages) +
         " pages for your searched query : " + queryStr.toString() + "</p>";
  script = "<script>$(\".ui-page-active [data-role='header'] h1\").html(\"" +
           QString::number(totalTracks) + " downloaded songs\");</script>";
  ...
  Next = "<a class='ui-btn ui-mini ui-icon-arrow-r ui-btn-icon-right ui-shadow "
         ...
         QString(" style='float:right;width:40%;background-color: rgba(36, "
                 "142, 179, 0.66);border: none;'") +
         " id='navBtn'" + " onclick='openSearchPagenumber(\"" +
         QString::number(currentPageNumber + 1) + "\",\"" +
         queryStr.toString() + "\")'" + ">Next</a>";
  Previous = "<a class='ui-btn ui-mini ui-icon-arrow-l ui-btn-icon-left "
             "ui-shadow ui-corner-all'" +
             QString(" style='float:left;width:40%;background-color: rgba(36, "
                     "142, 179, 0.66);border: none;'") +
  ...
  if (totalPages > 0) {
    QString html, li;
    int offset = pageNumber * limit;
    foreach (QStringList trackList,
             get_search_local_saved_tracks(offset, queryStr.toString())) {
...
void store::add_recently_played(QString trackId) {
  QSqlQuery query;
  bool added =
      query.exec("INSERT INTO recently_played('trackId','timestamp') VALUES('" +
                 trackId.trimmed() + "','" +
                 QString::number(QDateTime::currentMSecsSinceEpoch()) + "')");
  if (!added) {
    query.exec("UPDATE recently_played SET timestamp = '" +
               QString::number(QDateTime::currentMSecsSinceEpoch()) +
               "'  WHERE trackId = '" + trackId.trimmed() + "'");
  }
}
...
QList<QStringList> store::getRecentTrackList() {
  ...
  QList<QStringList> trackList;
  query.exec(
      "SELECT trackId FROM recently_played ORDER BY timestamp DESC LIMIT 50");
  if (query.record().count() > 0) {
    while (query.next()) {
...
void store::add_to_liked(QString trackId) {
  QSqlQuery query;
  query.exec("INSERT INTO liked_tracks('trackId','timestamp') VALUES('" +
             trackId.trimmed() + "','" +
             QString::number(QDateTime::currentMSecsSinceEpoch()) + "')");
}
...
bool store::is_liked_track(QString trackId) {
  QSqlQuery query;
  query.exec("SELECT trackId FROM liked_tracks WHERE trackId = '" +
             trackId.trimmed() + "';");
  if (query.next()) {
    return true;
  } else {
...
QList<QStringList> store::get_liked_tracks(int offset) {
  QSqlQuery query;
  QList<QStringList> trackList;
  query.exec("SELECT trackId FROM liked_tracks ORDER BY timestamp DESC LIMIT " +
             QString::number(limit) + " OFFSET " + QString::number(offset));
  while (query.next()) {
    trackList.append(getTrack(query.value("trackId").toString()));
  }
...
QString store::open_liked_tracks_PageNumber(int pageNumber) {
  ...
  head = "<p style='margin-top: -5px;'>showing page " +
         ...
         QString::number(totalPages) + " pages.</p>";
  script = "<script>$(\".ui-page-active [data-role='header'] h1\").html(\"" +
           QString::number(totalTracks) + " liked songs\");</script>";
  ...
  if (totalPages > 0) {
    ...
    foreach (QStringList trackList, get_liked_tracks(offset)) {
      ...
      QString divider = "!=-=!";
      QString offline = (isDownloaded(songId) == true)
                            ? "<span class='my-li-btn ui-icon-arrow-d "
                              "ui-btn-icon-notext'></span>"
                            : "";
      QString imgHtml, para;
      if (albumId.contains("undefined-")) {
        para = "<p style='margin-left: 7.5em;'>";
      ...
      if (!title.trimmed().isEmpty()) {
        li += "<li data-filtertext='" + title + " " + album + " " + artist +
              "' >" + "<a data-trackinfo='" + title + divider + artist +
              divider + album + divider + base64 + divider + songId + divider +
              albumId + divider + artistId + divider + "millis" +
              "' onclick='mainwindow.playLocalTrack(\"" + songId + "\")'>" +
              imgHtml + para + "" + title + "<br>" +
              "Album: " + QString(album == "undefined" ? "Youtube" : album) +
              "<br>" + "Artist: " + artist + "</p>" + offline + " </a>" +
              "<a href='#' onclick=\"track_option('" + songId +
              "')\">More Options</a>" + "</li>";
      }
    }
...
int store::getSearchResultTrackCountLikedSongs(QString queryStr) {
  QSqlQuery query;
  query.exec("SELECT * FROM tracks as t, liked_tracks as l where l.trackId = "
             "t.trackId and t.title LIKE '%" +
             queryStr + "%';");
  int count = 0;
  while (query.next()) {
    count++;
...
// main result provider update query in getSearchResultTrackCount also to get
// correct page numbers
QList<QStringList> store::get_search_liked_tracks(int offset,
                                                  QString queryStr) {
  QSqlQuery query;
  QList<QStringList> trackList;
  query.exec("SELECT * FROM tracks as t, liked_tracks as l where l.trackId = "
             "t.trackId and t.title LIKE '%" +
             queryStr + "%' LIMIT " + QString::number(limit) + " OFFSET " +
...
QString store::open_search_liked_tracks(int pageNumber, QVariant queryStr) {
  ...
  if (totalPages > 0) {
    ...
    int offset = pageNumber * limit;
    foreach (QStringList trackList,
             get_search_liked_tracks(offset, queryStr.toString())) {
...
void store::cleanUp() {
  ...
  query.exec("SELECT trackId FROM recently_played ORDER BY timestamp DESC "
  ...
  for (int i = 0; i < trackToRemove.count(); i++) {
    QSqlQuery q;
    q.exec("SELECT trackId FROM tracks WHERE downloaded ='1' AND trackId='" +
           trackToRemove.at(i) + "';");
    while (q.next()) {
  ...
  foreach (QString trackId, trackToRemove) {
    ...
    if (deleteAlbum.numRowsAffected() > 0) {
      deleteAlbum.exec("DELETE FROM arts WHERE albumId = 'undefined-" +
                       trackId + "';");
      if (deleteAlbum.numRowsAffected() > 0) {
        deleteAlbum.exec("DELETE FROM album WHERE albumId = 'undefined-" +
                         trackId + "';");
        if (deleteAlbum.numRowsAffected() > 0) {
