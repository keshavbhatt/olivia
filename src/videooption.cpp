...
VideoOption::VideoOption(QWidget *parent, store *store, radio *radio_manager)
    : QWidget(parent), ui(new Ui::VideoOption) {
  ui->setupUi(this);
  ...
  used_fifo_file_path =
      this->radio_manager->used_fifo_file_path.split(".fifo").first() +
      "_videoPlayer" + ".fifo";
...
VideoOption::~VideoOption() { delete ui; }

void VideoOption::setMeta(QString songId) {
  resetVars();
  QStringList trackMetaList = store_manager->getTrack(songId);
  QString ytIds, title, artist, album, base64, dominantColor, albumId, artistId,
      url;
  ...
  if (album == "undefined") {
    ...
    ui->cover->setMaximumWidth(
        static_cast<int>(ui->track_widget->height() * 1.50));
  }
...
void VideoOption::resetVars() {
  foreach (QRadioButton *rBtn,
           ui->quality_box->findChildren<QRadioButton *>()) {
    rBtn->deleteLater();
  }
...
void VideoOption::setMetaFromWeb(QVariant data) {
  ...
  QString ytIds, title, artist, album, coverUrl, songId, base64, dominantColor,
      artistId, albumId;
...
void VideoOption::processYtdlQueue() {
  ...
  if (ytdlQueue.count() > 0) {
    ...
    if (ytdlProcess == nullptr) {
      ...
      QString addin_path =
          QStandardPaths::writableLocation(QStandardPaths::DataLocation);
      ytdlProcess->start("python3", QStringList() << addin_path + "/core"
                                                  << "-F" << url);
      ui->progressBar->show();
...
void VideoOption::ytdlFinished(int code) {
  ...
  if (code == 0) {
    ...
    QByteArray b = senderProcess->readAll();
    QString s_data = QTextCodec::codecForMib(106)->toUnicode(b).trimmed();

    QStringList formats;
    if (!s_data.isEmpty()) {
      formats = s_data.split("resolution", QString::KeepEmptyParts , Qt::CaseInsensitive).last().split("\n");
    }

    foreach (QString format, formats) {
      if (format.contains("audio only",Qt::CaseInsensitive) && !format.isEmpty()) {
        ...
        size = format.split("|").at(1).trimmed().split(" ").first();
        qDebug()<<format<<endl<<format.split("|")<<endl;
        QRadioButton *button =
            new QRadioButton(resolution + "(" + size + ")", this);
        button->setObjectName("audio-" + code);
        connect(button, SIGNAL(clicked(bool)), this,
                SLOT(update_audio_video_code(bool)));
        ui->quality_box_layout_audio->addWidget(button);
      }

      if (format.contains("video only",Qt::CaseInsensitive) && !format.isEmpty()) {
        QString code, codec, resolution, size, fps;
        code = format.split(" ").first();
        codec = format.split(code + " ").last().trimmed().split(" ").first();
        resolution =
            format.split(codec + " ").at(1).trimmed().split(" ").first();
        fps = format.split(resolution + " ").at(1).trimmed().split(" ").first();
        size = format.split("|").at(1).trimmed().split(" ").first();
        if (!resolution_List.contains(resolution)) {
          QRadioButton *button = new QRadioButton(
              resolution + "(" + size + " " + fps + "fps)", this);
...
void VideoOption::getUrlForFormatsAndPLay(QString audioFormat,
                                          QString videoFormat) {
  QString addin_path =
      QStandardPaths::writableLocation(QStandardPaths::DataLocation);
  ...
  task = "play";
  connect(audioP, SIGNAL(finished(int)), this,
          SLOT(getUrlProcessFinished(int)));

  audioP->start("python3", QStringList()
                               << addin_path + "/core"
...
void VideoOption::getUrlForFormatsAndDownload(QString audioFormat,
                                              QString videoFormat) {
  ui->progressBar->show();
  task = "download";
...
void VideoOption::mergeAndPlay(QString videoUrlStr, QString audioUrlStr) {
  ...
  connect(player, SIGNAL(finished(int)), this,
          SLOT(getUrlProcessFinished(int)));
  player->start("mpv", QStringList()
                           << "--geometry=800x600+0+0"
                           << "-wid=" + QString::number(this->winId())
                           << "--title=MPV for Olivia - " + currentTitle
                           << "--no-ytdl" << videoUrlStr
                           << "--audio-file=" + audioUrlStr
                           << "--input-ipc-server=" + used_fifo_file_path
