...
equalizer::equalizer(QWidget *parent) : QWidget(parent), ui(new Ui::equalizer) {
  ui->setupUi(this);
  ui->fake->hide();
  ...
  settingsObj = new QSettings(setting_path + "/eq.conf",
                              QSettings::NativeFormat, nullptr);
}
  ...
  foreach (QLabel *label, labels_list) {
    if (!label->objectName().contains("ig")) {
      ...
      if (labelText.length() < 7) {
        label->setMinimumWidth(oneCharWidth * label->text().length() +
                               oneCharWidth * 2);
      }
      label->setFont(QFont(label->font().family(), fSize));
      label->setText("<p align=\"center\" ><span style=\"font-size:10pt;\">" +
                     labelText + "</span></p>");
    }
  }
...
bool equalizer::eventFilter(QObject *obj, QEvent *event) {
  if (event->type() == QEvent::Wheel &&
      (obj->objectName().contains("m") || obj->objectName().contains("b"))) {
...
void equalizer::updateEqVal() {
  ...
  if (ui->balance->value() > 10 || ui->balance->value() < 10) {
    ...
    eq_args +=
        "lavfi=[pan=stereo|c0=" + c1vStr + "*c0|c1=" + c2vStr + "*c1]" + ",";
  }

  // prepare master EQ bands args
  //  QString bandName = slider->objectName().split("b").last();
  eq_args += "equalizer=f=64:width_type=o:w=3.3:g=" +
             QString::number(ui->m1->value()) + "," +
             "equalizer=f=400:width_type=o:w=2.0:g=" +
             QString::number(ui->m2->value()) + "," +
             "equalizer=f=1250:width_type=o:w=1.3:g=" +
             QString::number(ui->m3->value()) + "," +
             "equalizer=f=2830:width_type=o:w=1.0:g=" +
             QString::number(ui->m4->value()) + "," +
             "equalizer=f=5600:width_type=o:w=1.0:g=" +
             QString::number(ui->m5->value()) + "," +
             "equalizer=f=12500:width_type=o:w=1.3:g=" +
             QString::number(ui->m6->value());

  // prepare super EQ bands args
  QString superEq =
      validBandVal(ui->b1) + validBandVal(ui->b2) + validBandVal(ui->b3) +
      validBandVal(ui->b4) + validBandVal(ui->b5) + validBandVal(ui->b6) +
      validBandVal(ui->b7) + validBandVal(ui->b8) + validBandVal(ui->b9) +
      validBandVal(ui->b10) + validBandVal(ui->b11) + validBandVal(ui->b12) +
      validBandVal(ui->b13) + validBandVal(ui->b14) + validBandVal(ui->b15) +
      validBandVal(ui->b16) + validBandVal(ui->b17) + validBandVal(ui->b18);
...
void equalizer::loadSettings() {
  ...
  foreach (QSlider *slider, sliders_list) {
    if (slider->objectName() != "balance" && slider->objectName() != "tempo") {
      slider->blockSignals(
          true); // block slider value changed cause it will cause eq_trigger
  ...
  setBandLabel(ui->tempo);
  ui->balance->setValue(
      settingsObj->value(ui->balance->objectName(), 10).toInt());
  setBandLabel(ui->balance);

  ui->tempo->blockSignals(false);
  ui->balance->blockSignals(false);

  ui->groupBox->setEnabled(
      settingsObj->value("equalizer_enabled", false).toBool());
  ui->groupBox_2->setEnabled(
      settingsObj->value("equalizer_enabled", false).toBool());

  ui->eq_checkBox->blockSignals(true);
  ui->eq_checkBox->setChecked(
      settingsObj->value("equalizer_enabled", false).toBool());
...
void equalizer::setBandLabel(QSlider *slider) {
  ...
  if (slider->objectName() == "tempo" && slider->value() > 0) {
    ...
    if (QString::number(d_val) == "1") {
      ui->ig2->setText("<p align=\"center\" ><span "
                       "style=\"font-size:10pt;\">Tempo(Normal)</span></p>");
    } else {
      ui->ig2->setText(
          "<p align=\"center\" ><span style=\"font-size:10pt;\">Tempo(" +
          QString::number(d_val) + ")</span></p>");
    }
  }
  ...
  if (slider->objectName() == "balance") {
    if (ui->balance->value() > 10 || ui->balance->value() < 10) {
      ...
      ui->balanceL->setText(
          "<p align=\"center\" ><span style=\"font-size:10pt;\">L(" + c1vStr +
          ")</span></p>");
      ui->balanceR->setText(
          "<p align=\"center\" ><span style=\"font-size:10pt;\">R(" + c2vStr +
          ")</span></p>");
    } else {
      // center
      ui->balanceL->setText("<p align=\"center\" ><span "
                            "style=\"font-size:10pt;\">L(1)</span></p>");
      ui->balanceR->setText("<p align=\"center\" ><span "
                            "style=\"font-size:10pt;\">R(1)</span></p>");
    }
  }
}
